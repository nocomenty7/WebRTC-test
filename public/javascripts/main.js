...
$(document).ready(function(){
    ...
    $(".flist-group-name").mouseover(function(){
	 ...
	 //side buttons 마우스 이벤트
    $(".button-wrap").children(".button, .exception")
	.mouseover(function(){
...
//framerate
    $(".button-framerate").click(function(){
		...
		var target = $(this);
		if(target.hasClass("frame-720")){
		    target.parent().siblings(".button").removeClass("frame-720 frame-480 frame-360 frame-240");
		    target.parent().siblings(".button").addClass("frame-720");
		}
		else if(target.hasClass("frame-480")){
		    target.parent().siblings(".button").removeClass("frame-720 frame-480 frame-360 frame-240");
		    target.parent().siblings(".button").addClass("frame-480");
		}
		else if(target.hasClass("frame-360")){
		    target.parent().siblings(".button").removeClass("frame-720 frame-480 frame-360 frame-240");
		    target.parent().siblings(".button").addClass("frame-360");
		}
		else if(target.hasClass("frame-240")){
		    target.parent().siblings(".button").removeClass("frame-720 frame-480 frame-360 frame-240");
		    target.parent().siblings(".button").addClass("frame-240");
	...
	// 알림 메시지 이벤트
    //$("#notification-number, .header-notifications")
	//.mouseover(function(){$(".header-notifications").css('background-color','#353535');})
	//.mouseout(function(){$(".header-notifications").css('background-color','rgba(0,0,0,0)');});
...
/*	 // 화상회의 시작 종료 이벤트 (임의로 테스트 용도로 해둔것 )삭제해도 됨
...
*/
	...
	.mouseover(function(){
    	...
    	.mouseout(function(){$(this).children(".button-delete").addClass("hidden");});
    ...
    $(".button-invite").click(function(){
    	...
    	if(isjoined == false)
            return;
    	$("#popover-invite-wrap").fadeIn("fast");
    });*/
    ...
    $.disableButton = function(){
		...
		for(var i = 1; i <= tableLength; i++){
		    var count = $("tbody tr:nth-child("+i+")").children(".noti-count").text();
		    if (count == 0){
				$("tbody tr:nth-child("+i+")").children(".noti-buttons").children(".noti-button-ok").removeClass("noti-button-active");
				$("tbody tr:nth-child("+i+")").children(".noti-buttons").children(".noti-button-ok").addClass("noti-button-disable");
		    } else if(count > 0) {
		    	$("tbody tr:nth-child("+i+")").children(".noti-buttons").children(".noti-button-ok").addClass("noti-button-active");
				$("tbody tr:nth-child("+i+")").children(".noti-buttons").children(".noti-button-ok").removeClass("noti-button-disable");
		    }
		}
    };
    ...
    //알림 메시지 마우스 오버
    $("tbody tr")
	.mouseover(function(){$(this).css("background-color","#FFF");})
			...
			$.ajax({
	        	...
	        	success: function(data) {
	            	...
	            	$( "#friends-search-input" ).autocomplete({
						...
						if(item.alreadyFriend){
							var returnval = $( "<li class='auto-user-list auto-user-list-friend'>" ).append( "<a class='s-friend'><div class='thin-box'></div><div class='s-content '><span class='s-mark'>친구</span><p class='name'>" + item.desc + "</p><p class='email'>" + item.label + "</p></div></a>" ).appendTo( ul ); 
							$(".auto-user-list-friend").click(function(e){
								e.stopPropagation();
					    	});
							return returnval;
						} else {
							return $( "<li class='auto-user-list auto-user-list-not-friend'>" ).append( "<a><div class='thin-box'></div><div class='s-content '><span class='s-mark'>친구</span><p class='name'>" + item.desc + "</p><p class='email'>" + item.label + "</p></div></a>" ).appendTo( ul );
						}
			   		};
	...
	socket.on('firstMessageBox', function(messages) {
        ...
        for(var i=0; i<messages.length; i++) {
            ...
            textToInsert[a++] = '</td><td class="noti-msg"><span class="host-name">';
	...
	socket.on("someoneJoin", function(data){
        ...
        textToInsert[a++] = '<p class="enter"><span>';
        textToInsert[a++] = data;
		...
		if(!isStart && webcamallow){
            ...
            if(id >= 2) {
                ...
                pendingInterval = setInterval(function() {
                    if(offerflag <= id-1) {
                        if( (offerflag==1) && !completeoffer[1] ) {
                            pc[1] = new RTCPeerConnection(null, pc_constraints);
                            pc[1].onicecandidate = handleIceCandidate1;
                            pc[1].addStream(localStream);
                            pc[1].onaddstream = handleRemoteStreamAdded1;
                            pc[1].createOffer(setLocalAndSendMessage1, function(){});
                            completeoffer[1] = true;
                        }
                        else if( (offerflag==2) && !completeoffer[2] ) {
                            pc[2] = new RTCPeerConnection(null, pc_constraints);
                            pc[2].onicecandidate = handleIceCandidate2;
                            pc[2].addStream(localStream);
                            pc[2].onaddstream = handleRemoteStreamAdded2;
                            pc[2].createOffer(setLocalAndSendMessage2, function(){});
                            completeoffer[2] = true;
                        }
                        else if( (offerflag==3) && !completeoffer[3] ) {
                            pc[3] = new RTCPeerConnection(null, pc_constraints);
                            pc[3].onicecandidate = handleIceCandidate3;
                            pc[3].addStream(localStream);
                            pc[3].onaddstream = handleRemoteStreamAdded3;
                            pc[3].createOffer(setLocalAndSendMessage3, function(){});
                            completeoffer[3] = true;
                        }
                        else if( (offerflag==4) && !completeoffer[4] ) {
                            pc[4] = new RTCPeerConnection(null, pc_constraints);
                            pc[4].onicecandidate = handleIceCandidate4;
                            pc[4].addStream(localStream);
                            pc[4].onaddstream = handleRemoteStreamAdded4;
                            pc[4].createOffer(setLocalAndSendMessage4, function(){});
                            completeoffer[4] = true;
                        }

                        if( (offerflag==1) && completeanswer[1] )
                            offerflag++;
                        else if( (offerflag==2) && completeanswer[2] )
                            offerflag++;
                        else if( (offerflag==3) && completeanswer[3] )
                            offerflag++;
                        else if( (offerflag==4) && completeanswer[4] )
                            offerflag++;
                    }
                    else {
            ...
            */
            ...
            if(id >= 2) {
                ...
                while(i <= id-1) {
                    ...
                    if( (i==1) && completeanswer[1] )
                        i++;
                    else if( (i==2) && completeanswer[2] )
                        i++;
                    else if( (i==3) && completeanswer[3] )
                        i++;
                    else if( (i==4) && completeanswer[4] )
                        i++;
                }
            }
               */
...
function sendChatMessage() {
	if(document.getElementById('local-msg').value == "\n"){ 
        ...
        /*
		document.getElementById('chat-wrap').innerHTML += (
        	"<div class=\"msg-wrap local-msg-sended\">" +
            "<span class=\"name\">" +
            myName +
            "</span>" +
            "<span class=\"msg-time\"><time>" +
